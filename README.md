...
- 予算期間が月次または年次の場合、購入予定月が対象期間に入る商品だけを合計します。購入予定月が未設定の商品は対象期間外として扱います。
- 商品詳細の「購入済みにして記録」から家計簿へ支出を作れます。同じ商品は二重に家計簿へ記録しません。
